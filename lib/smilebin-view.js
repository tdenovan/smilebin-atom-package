...
class SmilebinAtomView {
  ...
  async createComment(commentText, emoticon, startLine, endLine) {
    ...
    const range = editor.getSelectedScreenRange()
    const editorText = startLine === endLine ?
      editor.lineTextForBufferRow(startLine - 1) :
      editor.getTextInBufferRange(range)

    await this.commentProvider.createComment(
      filePath, commentText, emoticon, startLine, endLine, editorText
    )
  }
  ...
  _setupSubscriptions() {
    ...
    this.subscriptions.add(
      this.workspace.onDidStopChangingActivePaneItem(() => this._fetchAndShowComments())
    )
  }
  ...
  _renderComments(comment) {
    ...
    const range = [ [ comment.startLineNumber - 1, 0 ], [ comment.endLineNumber, 0 ] ]
    const marker = this._getEditor().markBufferRange(range)
    this._getEditor().decorateMarker(
      marker,
