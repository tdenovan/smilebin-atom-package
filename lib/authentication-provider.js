...
class AuthenticationProvider {
  ...
  /**
   * Attempts to sign in using a saved token (if one exists)
   */
  ...
  async _signIn(token) {
    try {
      ...
      // Sometimes we can have a valid jwt but it doesn't match to any user (e.g. user deleted from db)
      if (!currentUser) throw Error('No user for that token exists in db')
